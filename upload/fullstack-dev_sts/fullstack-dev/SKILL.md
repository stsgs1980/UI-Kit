...
This skill handles backend architecture, database, API, and project scaffolding. For visual and presentational work, delegate to:

| Companion Skill | When to Use | What It Covers |
|----------------|-------------|----------------|
| **frontend-design** | When starting a new page/component that needs a distinct visual direction | Design tokens, aesthetic direction, typography, color systems, layout composition, creative execution |
| **frontend-styling-expert** | When polishing existing UI, fixing CSS, adding animations, or improving accessibility | Responsive fixes, hover/focus states, animation choreography, WCAG compliance, cross-browser, dark mode |

**Typical combinations:**
- **Full project**: Use `fullstack-dev` for architecture/backend → `frontend-design` for visual system → `frontend-styling-expert` for polish
- **CRUD admin panel**: Use `fullstack-dev` for schema/API/routes → `frontend-styling-expert` for layout polish and accessibility
- **API-only task**: Use `fullstack-dev` alone (no companion needed)

**When NOT to use this skill alone:**
- If the user primarily asks about visual design, aesthetics, or "making it beautiful" — start with `frontend-design`
- If the user only needs CSS fixes, responsive tweaks, or accessibility — use `frontend-styling-expert`
